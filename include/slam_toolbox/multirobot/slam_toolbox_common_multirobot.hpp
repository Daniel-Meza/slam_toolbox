...
#ifndef SLAM_TOOLBOX__SLAM_TOOLBOX_COMMON_MULTIROBOT_HPP_
#define SLAM_TOOLBOX__SLAM_TOOLBOX_COMMON_MULTIROBOT_HPP_
...
class SlamToolboxMultirobot : public rclcpp::Node
{
public:
  explicit SlamToolboxMultirobot(rclcpp::NodeOptions);
  SlamToolboxMultirobot();
  virtual ~SlamToolboxMultirobot();
...
protected:
  ...
  std::vector<std::unique_ptr<message_filters::Subscriber<sensor_msgs::msg::LaserScan>>> scan_filter_subs_;
  ...
  std::map<std::string, tf2::Transform> m_map_to_odom_;
